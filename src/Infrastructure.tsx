...
import { infrastructure } from './fullQuestionBank'
import type { questionType } from './types'

function Infrastructure({nextSection, initialMessage}: {nextSection: (score: number) => void, initialMessage: string}) {
    const [isStart, setIsStart] = useState(true);
    
    let questions: questionType[] = [];
    let infraCopy = [...infrastructure];

    function getRandomQuestions(){
        let questionsLength = infraCopy.length;

        for(let i = 0; i < 10; i++){
            let random = Math.floor(Math.random() * questionsLength);
            questions.push(infraCopy.splice(random,1)[0]);
